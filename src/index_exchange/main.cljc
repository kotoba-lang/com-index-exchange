(ns index_exchange.main
  "Kotodama WASM entrypoint for the index_exchange clean-room actor (L4) — Clojure port.
  ...
  facts (`index_exchange.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "index_exchange")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"index_exchange.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "index_exchange-compat" :tier tier :entities entities} 200])
...
(defn start! [] :index_exchange-compat/ready)
